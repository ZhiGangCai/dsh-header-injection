...
请求头注入插件：按 host 规则注入 HTTP 请求头，**同名头覆盖原值**（`Headers.set` 语义），多条规则各自独立头集合，绕过目标站点客户端 WAF 拦截；其余请求零影响。默认规则对 `agentrouter.org` 注入 `User-Agent: RooCode/0.15.0`。

> 展示名：**请求头注入**（v0.2.0 起由「AgentRouter UA 注入 / UA注入」演进而来，从单一 UA 改写升级为任意请求头注入；包名与设置命名空间保持 `agent-router-ua` 不变）。
...
实测（2026-08-27）：仅把 `User-Agent` 改写为 `RooCode/0.15.0`，WAF 即放行进入令牌校验阶段（假 key 返回 `new_api_error` 而非 `unauthorized_client_error`），无需附加 X-Title / X-Stainless-* 等头。
...
注入语义：以原请求头为基础，规则内每条「头名: 值」经 `Headers.set` 覆盖（**同名头覆盖原值**，头名大小写不敏感），规则未提及的头保持不变。
...
```bash
# 本地目录安装（构建产物 lib/ 已入库）
dsh plugin --profile web add /path/to/agent-router-ua
```
...
插件默认即对 `agentrouter.org`（含 `co.agentrouter.org` 等子域）生效。可在 **DSH 设置 → 插件 → 请求头注入** 面板编辑并**即时生效**（无需重启）：
...
面板支持**添加 / 删除任意多条规则**，请求按规则顺序匹配、取第一条命中。保存时 Host 或请求头为空的规则会被丢弃（等于删除该条）；非法头名（非 RFC 7230 token）或空值的行会被忽略。

配置经 DSH 设置服务持久化到 `~/.dsh/settings.yaml` 的 `agent-router-ua:` 段；node half 通过 settings watch 实时接收变更，保存即生效。

**旧配置自动迁移**：v0.1.0 旧格式 `{enabled, hosts, ua}`（单 host 列表 + 单 UA）与 v0.2.0 中间格式 `rules: [{hosts, ua}]` 在加载时自动迁移为 `User-Agent` 头规则，已存的自定义 host 与 UA 值不会丢失。
...
- 状态摘要：DSH 命令 `agentrouter-ua`（输出当前规则列表与命中/改写计数）
- 健康路由：`GET /agent-router-ua/health`（含规则数组、命中/改写计数）
