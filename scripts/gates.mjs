/**
 * gates.mjs — agent-router-ua 轻量一致性门禁。
 *
 * 校验对象（不依赖 tsc 全量编译）：
 ...
 *      （headers.set）、旧配置迁移（normalizeCfg）、agentrouter.org 默认
...
check('包名 agent-router-ua', pkg.name === 'agent-router-ua')
...
check('node half：插件名预埋', js.includes('agent-router-ua'))
...
check('node half：旧配置迁移 normalizeCfg', js.includes('normalizeCfg'))
...
check('client half：id 正确', client.includes(`id: 'agent-router-ua'`))
check('client half：设置卡片改名 请求头注入', client.includes('请求头注入'))
check('client half：多规则编辑（添加规则）', client.includes('添加规则'))
check('client half：头集合多行编辑 textarea', client.includes('textarea'))
check('client half：旧格式快照迁移', client.includes('snapshotToForm'))
